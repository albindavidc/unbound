...
module.exports = {
  ...
  checkoutDetailsReload: async (req, res) => {
    ...
    for (const prod of userCart.items) {
      prod.price = prod.productId.bundleQuantity > prod.quantity ? prod.productId.bundlePrice : prod.productId.sellingPrice;
      const itemTotal = prod.price * prod.quantity;
      prod.itemTotal = itemTotal;
      totalPrice += itemTotal;
      totalPriceBeforeOffer += prod.price;
    ...
    let isCOD = totalPrice <= 1000;
    const userWallet = await Wallet.findOne({ userId: userId });
    let isInsufficient = totalPrice > (userWallet?.balance || 0);
    const payable = (await Cart.findOne({ userId }, { _id: 0, payable: 1 })).payable;
 
    // Send the updated details as partial view
    if (req.xhr || req.headers.accept.indexOf('json') > -1) {
      ...
      res.render('partials/user/checkoutDetails', {
        ...
        checkout: true,
        isInsufficient,
        layout: false, // Prevent full page render, only partial
        reload: true,
      });
    } else {
      // For full page load
